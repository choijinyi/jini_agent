...
2. ⚠**`cys send` 본문에 백틱을 쓰지 마라.** 큰따옴표 안 백틱은 bash 명령치환으로 실행되어
   그 구간이 빈 값으로 치환된 채 발신된다(보고가 조용히 훼손된다 — 오늘 3건 발생: master·CSO·나).
   코드·경로·명령을 인용할 때는 백틱 없이 쓰거나 작은따옴표로 감싸라. `2>&1 | tail` 로는
   안 보이니 「import: command not found」 류 흔적을 보고 알아채야 한다.
   정본: 장기기억 `reference_cys-send-backtick-command-substitution`.
   → 규약: **순환하는 노드에게 메시지로만 준 규칙은 없는 규칙이다.** 규칙은 장기기억·디렉티브·
   이 파일에 둔다. 그래서 이 항목이 여기 있다(다음 순환 후에도 살아남게).
3. worker(surface:56)가 커밋 `7db905e` 스냅샷으로 **독립 검증** 중이다. 그 반증이 오면 그것이 우선한다.
   ⚠단 그 스냅샷은 낡았다 — 현재 HEAD 는 `ffc76fe`(원격도 동일). 기준 커밋 재지정이 필요하다.
