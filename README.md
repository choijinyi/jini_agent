...
- **이 스킬들을 벤더 CLI 로 직접 부르면 우리 정책이 적용되지 않은 상류 본문을 읽는다.** jini 를 통하면
  `instruct`·`files`·`exec` 가 로컬 사본으로 가로채져 우리가 고친 본문이 쓰이지만, `npx @nomadamas/k-skill`
  을 직접 실행하는 경로는 우리 저장소 밖이라 손대지 않았다. 상류 본문에는 존재하지 않는 도구 호출 지시와
  결제·예약을 대신 완료하라는 문장이 그대로 있다.
