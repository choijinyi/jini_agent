...
자격증명은 사용자가 직접 입력한다. 에이전트가 대신 로그인하거나 자격증명을 저장하지 않는다 (jini 경계). generic fallback에서만 `KSKILL_SRT_ID`, `KSKILL_SRT_PASSWORD` 환경변수를 확인한다.
...
3. 실제 결제 버튼 직전에 승인 게이트(`write`·`edit`·`bash`·`git`)에서 열차, 날짜·시각, 승객, 좌석 등급, 총액을 보여주고 승인받는다.
