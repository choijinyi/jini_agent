...
```bash
npx -y @nomadamas/k-skill@0.2.2 exec korean-privacy-terms scripts/install.sh --
```
...
2. `npx -y @nomadamas/k-skill@0.2.2 exec korean-privacy-terms scripts/install.sh --` 로 업스트림을 dual-install 한다 (이미 설치돼 있으면 pin 확인만 한다).
...
```bash
npx -y @nomadamas/k-skill@0.2.2 read korean-privacy-terms scripts/upstream.pin
