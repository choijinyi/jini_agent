...
```bash
npx -y @nomadamas/k-skill@0.2.2 exec mfds-drug-safety scripts/mfds_drug_safety.py -- interview \
  --question "타이레놀이랑 판콜 같이 먹어도 되나요?" \
  --symptoms "두드러기와 어지러움"
```

```bash
npx -y @nomadamas/k-skill@0.2.2 exec mfds-drug-safety scripts/mfds_drug_safety.py -- lookup --item-name "타이레놀" --item-name "판콜"
```
